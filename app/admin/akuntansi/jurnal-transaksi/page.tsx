...
import { Eye, Edit, Trash2, Plus, Minus, Calendar, FileText } from "lucide-react";
...
export default function JurnalTransaksiPage() {
  ...
  const { data: selectedItemData, isLoading: isLoadingSelectedItem } = useGetJournalByIdQuery(
    selectedItemId!,
    {
      skip: !selectedItemId,
    }
  ...
  const getStatusBadge = (isPosted: boolean | number) => {
    const isPostedValue = typeof isPosted === 'boolean' ? isPosted : isPosted === 1;
  ...
  const addDetailEntry = () => {
    setFormData({
      ...formData,
      details: [...formData.details, { coa_id: 0, type: "debit", debit: 0, credit: 0, memo: "" }],
    });
  };
  ...
  const updateDetailEntry = (index: number, field: string, value: string | number) => {
      ...
      {journalData && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-700">
            Menampilkan {journalData.from} sampai {journalData.to} dari {journalData.total} data
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
       ...
       <Dialog open={isCreateModalOpen || isEditModalOpen} onOpenChange={(open) => {
         ...
         <DialogContent className="max-w-5xl max-h-[95vh] overflow-hidden flex flex-col">
           <DialogHeader className="flex-shrink-0">
             <DialogTitle>
               {selectedItem ? "Edit Jurnal Transaksi" : "Tambah Jurnal Transaksi"}
             </DialogTitle>
           </DialogHeader>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     ...
                     <div>
                       ...
                       <Select
                         ...
                         onValueChange={(value) => setFormData({ ...formData, is_posted: Number(value) })}
                       >
                         <SelectTrigger>
                 ...
                 <div className="bg-white border rounded-lg">
                   <div className="p-4 border-b">
                     <div className="flex items-center justify-between">
                       ...
                       <Button type="button" onClick={addDetailEntry} size="sm" className="bg-blue-600 hover:bg-blue-700">
                     ...
                     {formData.details.map((detail, index) => (
                       <div key={index} className="bg-gray-50 p-4 rounded-lg border">
                         <div className="flex items-center justify-between mb-3">
                           <span className="text-sm font-medium text-gray-700">Detail #{index + 1}</span>
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
                           <div className="md:col-span-2">
                             ...
                             <Select
                               ...
                               onValueChange={(value) => updateDetailEntry(index, "coa_id", Number(value))}
                             >
                               <SelectTrigger>
                           ...
                           <div>
                             ...
                             <Select
                               value={detail.type}
                               onValueChange={(value: "debit" | "credit") => updateDetailEntry(index, "type", value)}
                             >
                               <SelectTrigger>
                           ...
                           {detail.type === 'debit' ? (
                             <div>
                               ...
                               <Input
                                 ...
                                 onChange={(e) => {
                                   ...
                                   updateDetailEntry(index, "debit", Number(value) || 0);
                                 }}
                                 placeholder="0"
                               />
                             </div>
                             ...
                             <div>
                               ...
                               <Input
                                 ...
                                 onChange={(e) => {
                                   const value = e.target.value.replace(/[^0-9]/g, '');
                                   updateDetailEntry(index, "credit", Number(value) || 0);
                                 }}
                                 placeholder="0"
                               />
                             </div>
                           ...
                           <div className="md:col-span-2">
                             ...
                             <Input
                               value={detail.memo}
                               onChange={(e) => updateDetailEntry(index, "memo", e.target.value)}
                 ...
                 <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg border">
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                     <div className="space-y-2">
                       ...
                       <p className="text-lg font-semibold text-gray-900">{formatDate(selectedItemData.date)}</p>
                     </div>
                     <div className="space-y-2">
                       <Label className="text-sm font-medium text-gray-500">Status</Label>
                       <div className="mt-1">{getStatusBadge(selectedItemData.is_posted)}</div>
                     </div>
                     <div className="space-y-2">
                       <Label className="text-sm font-medium text-gray-500">Total Debit</Label>
                       <p className="text-lg font-bold text-green-600">
                         {formatCurrency(selectedItemData.details?.reduce((sum, detail) => {
                   ...
                   <div className="mt-4 pt-4 border-t border-gray-200">
                     ...
                     <p className="text-sm text-gray-700 mt-1">{selectedItemData.description}</p>
                   </div>
                   
                   <div className="mt-4 flex items-center justify-between text-sm text-gray-500">
                     <span>Dibuat: {formatDate(selectedItemData.created_at)}</span>
                     <span>{selectedItemData.details?.length || 0} entri detail</span>
                   </div>
                 </div>

                 {/* Detail Entries */}
                 {selectedItemData.details && selectedItemData.details.length > 0 ? (
                   <div className="bg-white border rounded-lg">
                               ...
                               <td className="px-6 py-4 text-sm text-gray-900">
                                 <div className="max-w-xs">
                                   <p className="font-medium">
                                     {detail.coa ? `${detail.coa.code} - ${detail.coa.name}` : getCOAName(detail.coa_id)}
                                   </p>
                                 </div>
                               </td>
                               <td className="px-6 py-4 whitespace-nowrap">
                                 <Badge 
                                   variant={detail.type === 'debit' ? 'default' : 'secondary'}
                                   className={detail.type === 'debit' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}
                                 >
                   ...
                   <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                     ...
                     <h3 className="text-lg font-medium text-gray-900 mb-2">Tidak Ada Detail</h3>
                     <p className="text-gray-500">Jurnal ini belum memiliki detail entri.</p>
                   </div>
                 )}
               </div>
             </div>
